...
export const PostTags = ({className, data, variant, ...props}: PostTagsProps) => {
  const {tags} = readFragment(PostTagsFragment, data)
  return (
    <div className={cn("flex flex-none flex-wrap items-center justify-center gap-1", className)} {...props}>
...
export const PostExtras = ({className, data, variant, ...props}: PostExtrasProps) => {
  const {publishedAt, readTimeInMinutes, views} = readFragment(PostExtrasFragment, data)
  const ago = D(publishedAt).fromNow()

  return (
    <Badge variant={variant} className={cn("justify-center gap-1", className)} {...props}>
      <span>{ago}</span>
      <span>·</span>
